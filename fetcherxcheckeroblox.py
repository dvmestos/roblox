...
def load_proxies():
    global proxies
    source = input("Enter proxy source (file/link): ").strip().lower()
    if source == "file":
        with open('proxies.txt', 'r') as f:
            proxies = [line.strip() for line in f.readlines() if line.strip()]
    elif source == "link":
        url = input("Enter proxy link: ").strip()
        try:
            response = requests.get(url)
            if response.status_code == 200:
                proxies = response.text.strip().split('\n')
                print(f"Loaded {len(proxies)} proxies from link.")
            else:
                print("Failed to fetch proxies from link.")
...
def check_code(code, proxy_mode):
    url = "https://apis.roblox.com/payments-gateway/v1/gift-card/redeem"
    headers = {
...
def fetcher(count, proxy_mode):
    for _ in range(count):
        code = generate_code()
        print(f"[FETCHED] {code}")
        save_code(code)

def checker(proxy_mode):
...
def main():
    mode = input("Select mode (fetcher/checker): ").strip().lower()
    proxy_mode = input("Use proxies? (yes/no): ").strip().lower() == 'yes'
